    ...
    module Type
      ...
      module Union
        ...
        private def resolve_variant(value)
          ...
          in [_, ModernTreasury::Internal::Type::BaseModel]
            value.class
          in [Symbol, Hash]
            key = value.fetch(@discriminator) do
              value.fetch(@discriminator.to_s, ModernTreasury::Internal::OMIT)
            end
        ...
        def inspect(depth: 0)
          ...
          "#{name}[#{variants.map { ModernTreasury::Internal::Type::Converter.inspect(_1, depth: depth.succ) }.join(' | ')}]"
          # rubocop:enable Layout/LineLength
        end
      end
    end
