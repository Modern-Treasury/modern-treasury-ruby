...
module ModernTreasury
  ...
  module Converter
    ...
    #
    #   @option spec [Proc, ModernTreasury::Converter, Class] :enum
    #
    #   @option spec [Proc, ModernTreasury::Converter, Class] :union
    #
    ...
    def self.type_info(spec)
      ...
      in Proc
        spec
      in ModernTreasury::Converter | Class
        -> { spec }
      in true | false
  ...
  class Union
    ...
    #
    def self.==(other)
      other.is_a?(Class) && other <= ModernTreasury::Union && other.variants == variants
    end
  ...
  class ArrayOf
    ...
    def coerce(value)
      ...
      in Enumerable unless value.is_a?(Hash)
        value.map { |item| ModernTreasury::Converter.coerce(type, item) }
      else
        value
      end
    end
    ...
    def dump(value)
      ...
      case value
      in Enumerable unless value.is_a?(Hash)
        value.map { |item| ModernTreasury::Converter.dump(type, item) }.to_a
      else
        value
      end
    end
  ...
  class BaseModel
    ...
    #
    # @param type_info [Proc, ModernTreasury::Converter, Class, Hash{Symbol=>Object}, nil]
    #
    # @param spec [Hash{Symbol=>Object}, Proc, ModernTreasury::Converter, Class] .
    #
    #   @option spec [NilClass, TrueClass, FalseClass, Integer, Float, Symbol] :const
    ...
    #
    def self.coerce(value)
      case (coerced = ModernTreasury::Util.coerce_hash(value))
      in Hash
        new(coerced)
      else
        value
    ...
    #
    def initialize(data = {})
      case (coerced = ModernTreasury::Util.coerce_hash(data))
      in Hash
