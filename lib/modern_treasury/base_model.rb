...
module ModernTreasury
  ...
  module Converter
    ...
    class << self
      # @api private
      #
      # @param spec [Hash{Symbol=>Object}, Proc, ModernTreasury::Converter, Class] .
      #
      ...
      def type_info(spec)
        ...
        in ModernTreasury::Converter | Class | Symbol
          -> { spec }
      ...
      #
      # @param target [ModernTreasury::Converter, Class]
      #
      # @param value [Object]
      #
      ...
      def dump(target, value)
        target.is_a?(ModernTreasury::Converter) ? target.dump(value) : ModernTreasury::Unknown.dump(value)
      end
    end
  end
  ...
  class BooleanModel
    ...
    #
    # @return [Boolean]
    def self.==(other) = other.is_a?(Class) && other <= ModernTreasury::BooleanModel
  ...
  module Enum
    ...
    # @return [Boolean]
    def ==(other)
      other.is_a?(Module) && other.singleton_class <= ModernTreasury::Enum && other.values.to_set == values.to_set
    end
  ...
  module Union
    ...
    # @api private
    #
    # @param key [Symbol, Hash{Symbol=>Object}, Proc, ModernTreasury::Converter, Class]
    #
    # @param spec [Hash{Symbol=>Object}, Proc, ModernTreasury::Converter, Class] .
    #
    #   @option spec [NilClass, TrueClass, FalseClass, Integer, Float, Symbol] :const
    ...
    private def variant(key, spec = nil)
      variant_info =
        case key
        in Symbol
          [key, ModernTreasury::Converter.type_info(spec)]
        in Proc | ModernTreasury::Converter | Class | Hash
          [nil, ModernTreasury::Converter.type_info(key)]
        end
    ...
    # @param value [Object]
    #
    # @return [ModernTreasury::Converter, Class, nil]
    private def resolve_variant(value)
      case [@discriminator, value]
      in [_, ModernTreasury::BaseModel]
        value.class
      in [Symbol, Hash]
        key = value.fetch(@discriminator) do
          value.fetch(@discriminator.to_s, ModernTreasury::Util::OMIT)
        end

        return nil if key == ModernTreasury::Util::OMIT
    ...
    # @return [Boolean]
    def ==(other)
      other.is_a?(Module) && other.singleton_class <= ModernTreasury::Union && other.derefed_variants == derefed_variants
    end
    ...
    def coerce(value, state:)
      if (target = resolve_variant(value))
        return ModernTreasury::Converter.coerce(target, value, state: state)
      end
      ...
      known_variants.each do |_, variant_fn|
        ...
        coerced = ModernTreasury::Converter.coerce(target, value, state: state)
    ...
    def dump(value)
      if (target = resolve_variant(value))
        return ModernTreasury::Converter.dump(target, value)
      end

      known_variants.each do
        target = _2.call
        return ModernTreasury::Converter.dump(target, value) if target === value
      end
  ...
  class ArrayOf
    ...
    # @param type_info [Hash{Symbol=>Object}, Proc, ModernTreasury::Converter, Class]
    #
    ...
    #
    # @return [Boolean]
    def ==(other) = other.is_a?(ModernTreasury::ArrayOf) && other.nilable? == nilable? && other.item_type == item_type
      ...
      value
        .map do |item|
          ...
          in [true, nil]
            ...
            ModernTreasury::Converter.coerce(target, item, state: state)
          end
        end
    end
    ...
    def dump(value)
      target = item_type
      value.is_a?(Array) ? value.map { ModernTreasury::Converter.dump(target, _1) } : super
    end

    # @api private
    #
    # @return [ModernTreasury::Converter, Class]
    ...
    def initialize(type_info, spec = {})
      @item_type_fn = ModernTreasury::Converter.type_info(type_info || spec)
      @nilable = spec[:nil?]
    end
  end
  ...
  class HashOf
        ...
        .to_h do |key, val|
          ...
          v =
            ...
            in [true, nil]
              ...
              ModernTreasury::Converter.coerce(target, val, state: state)
            end
    ...
    def dump(value)
      target = item_type
      value.is_a?(Hash) ? value.transform_values { ModernTreasury::Converter.dump(target, _1) } : super
    end
  ...
  class BaseModel
    ...
    class << self
      ...
      private def add_field(name_sym, required:, type_info:, spec:)
        type_fn, info =
          case type_info
          in Proc | ModernTreasury::Converter | Class
            [ModernTreasury::Converter.type_info({**spec, union: type_info}), spec]
          in Hash
            [ModernTreasury::Converter.type_info(type_info), type_info]
          end
        ...
        define_method(name_sym) do
          target = type_fn.call
          value = @data.fetch(name_sym) { const == ModernTreasury::Util::OMIT ? nil : const }
          state = {strictness: :strong, exactness: {yes: 0, no: 0, maybe: 0}, branched: 0}
          if (nilable || !required) && value.nil?
            nil
          else
            ModernTreasury::Converter.coerce(
              target,
              value,
              state: state
            )
    ...
    class << self
      ...
      def coerce(value, state:)
        ...
        unless (val = ModernTreasury::Util.coerce_hash(value)).is_a?(Hash)
          exactness[:no] += 1
          return value
        end
        exactness[:yes] += 1
        ...
        fields.each do |name, field|
          ...
          unless val.key?(api_name)
            if const != ModernTreasury::Util::OMIT
              exactness[:yes] += 1
          ...
          converted =
            if item.nil? && (nilable || !required)
              ...
              coerced = ModernTreasury::Converter.coerce(target, item, state: state)
              case target
              in ModernTreasury::Converter | Symbol
                coerced
              else
                item
              end
      ...
      def dump(value)
        unless (coerced = ModernTreasury::Util.coerce_hash(value)).is_a?(Hash)
          return super
        end
        ...
        coerced.each do |key, val|
          ...
          in nil
            ...
            in :coerce
              ...
              target = type_fn.call
              acc.store(api_name, ModernTreasury::Converter.dump(target, val))
            end
          end
        end
        ...
        known_fields.each_value do |field|
          ...
          next if mode == :coerce || acc.key?(api_name) || const == ModernTreasury::Util::OMIT
    ...
    def initialize(data = {})
      case ModernTreasury::Util.coerce_hash(data)
      in Hash => coerced
        @data = coerced
      else
