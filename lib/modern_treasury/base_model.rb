...
module ModernTreasury
  ...
  module Converter
    # @private
    # @param spec [Proc, Class, ModernTreasury::Converter, Hash]
    #
    # @return [Proc]
    #
    def self.type_info(spec)
      ...
      in Proc
        spec
      in Class | ModernTreasury::Converter
        -> { spec }
      in true | false
    ...
    #
    # @param target [Class, ModernTreasury::Converter]
    # @param value [Object]
    #
    # @return [Object]
    #
    def self.coerce(target, value)
      ...
      in Class
        case target
        in -> { _1 <= ModernTreasury::Converter }
          target.coerce(value)
        in -> { _1 <= NilClass }
          nil
        in -> { _1 <= Integer }
    ...
    def self.dump(target, value)
      case target
      in ModernTreasury::Converter | -> { _1.is_a?(Class) && _1.include?(ModernTreasury::Converter) }
        target.dump(value)
      else
        value
      end
  ...
  class Union
    ...
    # @private
    #
    # @param key [Symbol, ModernTreasury::Converter, Proc]
    # @param variant [Proc, Object]
    # @param key [Symbol, nil]
    #
    # @return [void]
    ...
    private_class_method def self.variant(key, spec = {})
      variant_info =
        ...
        in Proc | Class | ModernTreasury::Converter | Hash
  ...
  class ArrayOf
    ...
    def initialize(type_info, spec = {})
      @items_type_fn = ModernTreasury::Converter.type_info(type_info || spec)
    end
  end
    ...
    def coerce(value)
      ...
      in Hash
        value.to_h do |key, val|
          coerced = ModernTreasury::Converter.coerce(items_type, val)
    ...
    def dump(value)
      ...
      in Hash
        value.transform_values do |val|
          ModernTreasury::Converter.dump(items_type, val)
        end
      else
        value
      end
  ...
  class BaseModel
    ...
    # @private
    #
    # @param data [ModernTreasury::BaseModel, Hash{Symbol => Object}]
    #
    ...
    #
    def self.fields
      @fields ||= (superclass == ModernTreasury::BaseModel ? {} : superclass.fields.dup)
    end
    ...
    # @param data [Hash{Symbol => Object}, ModernTreasury::BaseModel] Raw data to initialize the model with.
    #
    def initialize(data = {})
