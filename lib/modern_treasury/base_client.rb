...
module ModernTreasury
  ...
  class BaseClient
    ...
    def initialize(
      ...
      @base_path = ModernTreasury::Util.normalize_path(base_url_parsed.path)
    ...
    def validate_request(req, opts)
      ...
      case opts
      in ModernTreasury::RequestOptions | Hash
        valid_keys = ModernTreasury::RequestOptions.options
    ...
    def resolve_uri_elements(req)
      from_args =
        if (url = req[:url])
          ModernTreasury::Util.parse_uri(url)
        else
          path = ModernTreasury::Util.normalize_path("/#{@base_path}/#{req.fetch(:path)}")
    ...
    def prep_request(options)
      ...
      if [:get, :head, :options].include?(method)
        ...
        body = ModernTreasury::Util.deep_merge(body, extra_body)
      end
    ...
    def follow_redirect(request, status:, location_header:)
      uri = ModernTreasury::Util.unparse_uri(request)
      location =
        ModernTreasury::Util.suppress(ArgumentError) do
      ...
      # from undici
      if ModernTreasury::Util.uri_origin(uri) != ModernTreasury::Util.uri_origin(location)
    ...
    # @return [String]
    def inspect
      base_url = ModernTreasury::Util.unparse_uri(scheme: @scheme, host: @host, port: @port, path: @base_path)
  ...
  module HTTP
    class Error < ModernTreasury::Error
    end
